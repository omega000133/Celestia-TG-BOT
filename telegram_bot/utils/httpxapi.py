    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"error {response.status_code}: {response.reason_phrase} llamado desde {caller_name}, archivo {file_name}, línea {line_no}"
                )
                return None
    except Exception as e:
        print(
            f"Error al obtener los datos de la API: {e} llamado desde {caller_name}, archivo {file_name}, línea {line_no}"
        )
        return None
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.post(
                url, json=data, follow_redirects=True, timeout=5
            )
            if response.status_code == 200:
                return response.json()
            else:
...
def obtener_info_llamante():
    ...
    archivo_actual = os.path.abspath(
        archivo_actual
    )  # Normaliza la ruta para comparación precisa
