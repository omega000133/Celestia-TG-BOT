...
@timer
def get_apr(update: Update, context: CallbackContext):
    try:
        ...
        if annual_provisions is not None and bonded_tokens is not None and community_tax is not None:
            nominal_apr = calcular_nominal_apr(annual_provisions, bonded_tokens, community_tax)
            print(f"Nominal APR: {nominal_apr:.2f}%")
            update.message.reply_text(f"Nominal APR: {nominal_apr:.2f}%")
        else:
            update.message.reply_text("No se pudo calcular el APR debido a un error al obtener los datos.")
    except Exception as e:
        update.message.reply_text(f"Error al obtener el APR: {e}")
