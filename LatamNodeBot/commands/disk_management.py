...
def diskalert_os(update: Update, context: CallbackContext):
    # Lógica para el comando diskalert...
    # Ejecutar el comando para obtener el espacio libre en disco
    try:
        comando_df = ['df', '-BG', '/']
        resultado_df = subprocess.run(comando_df, capture_output=True, text=True, check=True)
        ...
        update.message.reply_text("Error al verificar el espacio en disco.")
    ...
    if int(espacio_libre) < MIN_FREE_SPACE:
        update.message.reply_text(f"Alerta de espacio en disco bajo: Solo quedan {espacio_libre_gb} GB libres en el servidor.")
    else:
        update.message.reply_text(f"Espacio en disco suficiente: {espacio_libre_gb} GB libres.")

@timer
def diskalert_py(update: Update = None, context: CallbackContext = None, path="/", porcentaje=PORCENTAJE):
    """
    ...
    if update is not None and context is not None:
        update.message.reply_text(alert_message)
    else:
        print(alert_message)
